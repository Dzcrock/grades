...
class SchoolAnalyticsApp:
    ...
    def create_classes_tab(self):
        ...
        # Список классов
        list_frame = ttk.Frame(self.classes_frame)
        list_frame.pack(side=tk.LEFT, fill=tk.Y, padx=5, pady=5)

        ttk.Label(list_frame, text="Список классов", style='Heading.TLabel').pack(anchor=tk.W)

        self.class_listbox = tk.Listbox(list_frame, height=15, width=30)
        self.class_listbox.pack(fill=tk.BOTH, expand=True)
        self.class_listbox.bind('<<ListboxSelect>>', self.on_class_select)

        # Правая панель с информацией о классе
        right_panel = ttk.Frame(self.classes_frame)
        right_panel.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=5, pady=5)

        # Текстовая информация о классе
        self.class_info_text = scrolledtext.ScrolledText(right_panel, height=15, font=('Consolas', 10))
        self.class_info_text.pack(fill=tk.BOTH, expand=True, pady=5)

        # График класса
        self.class_figure = plt.Figure(figsize=(8, 4), dpi=100)
        self.class_canvas = FigureCanvasTkAgg(self.class_figure, right_panel)
    ...
    def show_class_info(self, class_name):
        ...
        lines.append("=" * 60)
        lines.append(f"👥 Учеников: {stats['count']}")
        lines.append(f"📊 Средний балл класса: {stats['avg_class_mean']:.2f}")
        lines.append(f"📈 Лучший средний: {stats['max_mean']:.2f}")
        lines.append(f"📉 Худший средний: {stats['min_mean']:.2f}")

        lines.append("\n📊 Распределение по успеваемости:")
        dist = stats['distribution']
        lines.append(f"  🏆 Отличники (≥4.5): {dist['excellent']}")
        lines.append(f"  📚 Хорошисты (4-4.5): {dist['good']}")
        lines.append(f"  📖 Троечники (3-4): {dist['satisfactory']}")
        lines.append(f"  ⚠️ Неуспевающие (<3): {dist['poor']}")

        lines.append("\n📝 Список учеников:")
        for student in sorted(students, key=lambda x: x.statistics['mean'], reverse=True):
            mean = student.statistics['mean']
            mark = "✅" if mean >= 4 else "📖" if mean >= 3 else "⚠️"
            lines.append(f"  {mark} {student.name}: {mean:.2f}")

        self.class_info_text.delete(1.0, tk.END)
        self.class_info_text.insert(1.0, "\n".join(lines))

        # График
        self.plot_class_chart(class_name, students)

    def plot_class_chart(self, class_name, students):
        """Построение графика для класса"""
        ...
        for student in sorted(students, key=lambda x: x.statistics['mean']):
            names.append(student.name.split()[0] if student.name else "Ученик")
        ...
        bars = ax.barh(names, means, color=colors, edgecolor='black')

        ax.set_xlabel('Средний балл')
        ax.set_title(f'Успеваемость класса {class_name}')
        ax.axvline(x=np.mean(means), color='red', linestyle='--',
                   label=f'Среднее по классу: {np.mean(means):.2f}')
        ax.legend()
    ...
    def update_analytics(self):
        ...
        if hasattr(self, 'classes') and self.classes:
            ...
            for class_name in sorted(self.classes.keys()):
                students = self.classes[class_name]
                stats = self.get_class_statistics()[class_name]
                display = f"{class_name} ({stats['count']} уч.)"
    ...
    def process_data(self):
        ...
        try:
            ...
            first_col = self.data.columns[0]
            self.data = self.data[self.data[first_col].notna()]
            self.data = self.data[self.data[first_col].astype(str).str.strip() != '']
            self.data = self.data[~self.data[first_col].astype(str).str.lower().isin(['nan', 'none', ''])]
            ...
            if len(self.data) == 0:
                self.log("⚠️ Нет валидных данных после фильтрации")
                return

            # Поиск колонки с классом
            class_col_idx, class_location, header_class = self.find_class_column(self.data)
            ...
            self.students = []
            self.classes = {}
            class_info = header_class if class_location == 'header' else None

            for idx, row in self.data.iterrows():
                try:
                    # Получаем ФИО из первой колонки
                    name = str(row[first_col]).strip()
            ...
            for student in self.students:
                if student.student_class:
                    class_key = student.student_class
                    if class_key not in self.classes:
                        self.classes[class_key] = []
                    self.classes[class_key].append(student)
    ...
    def find_class_column(self, df):
        ...
        # Проверяем первую колонку (может быть ФИО + класс)
        first_col_values = df.iloc[:, 0].astype(str).tolist() if len(df) > 0 else []

        # Поиск отдельной колонки с классом
        for idx, col in enumerate(df.columns):
            if ClassParser.is_class_column(col):
                return idx, 'column', None  # Возвращаем 3 значения
        ...
        # Поиск класса в первой колонке (ФИО + класс)
        for val in first_col_values[:20]:  # Проверяем первые 20 строк
            class_name = ClassParser.extract_class_from_string(val)
            if class_name:
                # Убираем класс из ФИО
                clean_name = re.sub(r'\s*[,\(\)]?\s*' + re.escape(class_name) + r'\s*[,\(\)]?', '', val,
                                    flags=re.IGNORECASE).strip()
                if clean_name and len(clean_name) > 3:  # Проверяем, что осталось имя
        ...
        return None, None, None  # Всегда возвращаем 3 значения
...
def update_students_list(self):
    ...
    for student in self.students:
        ...
        if student.student_class != current_class:
            current_class = student.student_class
            if current_class:
                self.students_listbox.insert(tk.END, f"📚 КЛАСС {current_class}")
                self.students_listbox.itemconfig(tk.END, fg='blue', font=('Arial', 10, 'bold'))
...
def filter_students(self, *args):
    ...
    for student in self.students:
        if search_term in student.name.lower() or (
                student.student_class and search_term in student.student_class.lower()):
            # Добавляем разделитель для нового класса
            if student.student_class != current_class:
                current_class = student.student_class
                if current_class:
...
def get_class_statistics(self):
    ...
    try:
        for class_name, students in self.classes.items():
            ...
            class_means = [s.statistics.get('mean', 0) for s in students if s.statistics]
            all_grades = []
            for s in students:
                if s.grades:
                    all_grades.extend([g for g in s.grades if g is not None])
            ...
            stats = {
                'count': len(students),
                'avg_class_mean': np.mean(class_means),
                'min_mean': min(class_means),
                'max_mean': max(class_means),
                'avg_grade': np.mean(all_grades) if all_grades else 0,
                'students': students
            }

            # Распределение по успеваемости
            excellent = len([s for s in students if s.statistics and s.statistics.get('mean', 0) >= 4.5])
            good = len([s for s in students if s.statistics and 4 <= s.statistics.get('mean', 0) < 4.5])
            satisfactory = len([s for s in students if s.statistics and 3 <= s.statistics.get('mean', 0) < 4])
            poor = len([s for s in students if s.statistics and s.statistics.get('mean', 0) < 3])
        ...
        self.log(f"⚠️ Ошибка в get_class_statistics: {e}")
