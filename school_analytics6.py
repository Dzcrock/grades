...
class GradeModeDialog:
    ...
    def show(self):
        ...
        # Подпись автора (ВНУТРИ функции, где dialog определен)
...
class VPRAnalytics:
    ...
    def load_file(self):
        ...
        try:
            ...
            self.data = self.clean_text_columns(self.data)

            if len(self.data.columns) <= 1:
                messagebox.showerror("Ошибка", "Не найдено колонок с оценками")
                return
            ...
            self.log(f"📝 Колонок с оценками: {len(self.data.columns) - 1}")
...
class Student:
    """Класс для хранения информации о конкретном ученике с автоматическим объединением"""
    ...
    def safe_get_statistic(student, stat_name, default=0):
        """Безопасное получение статистики ученика"""
        try:
            if hasattr(student, 'statistics') and student.statistics:
                return student.statistics.get(stat_name, default)
            elif hasattr(student, '_calculate_statistics'):
    ...
    def __init__(self, name, grades, student_class=None):
        self.name = name
        self.original_names = [name]  # Для отслеживания вариантов имени
    ...
    def merge_with(self, other_student):
        """Объединение двух записей одного ученика"""
        if self.name != other_student.name:
            # Сохраняем оба варианта имени
            self.original_names.extend(other_student.original_names)
            # Используем самое короткое имя как основное
            if len(other_student.name) < len(self.name):
                self.name = other_student.name

        # Объединяем оценки (берем максимальную длину)
        max_len = max(len(self.grades), len(other_student.grades))
        new_grades = []

        for i in range(max_len):
            grade1 = self.grades[i] if i < len(self.grades) else None
            grade2 = other_student.grades[i] if i < len(other_student.grades) else None

            # Если есть оценка в одной из записей, берем её
            if grade1 is not None and grade2 is not None:
                # Если обе оценки есть, берем среднее
                new_grades.append((grade1 + grade2) / 2)
            elif grade1 is not None:
                new_grades.append(grade1)
            elif grade2 is not None:
                new_grades.append(grade2)
            else:
                new_grades.append(None)

        self.grades = new_grades

        # Объединяем классы (если один из классов определен)
        if other_student.student_class and not self.student_class:
            self.student_class = other_student.student_class
...
class DataFilter:
    ...
    # Ключевые слова, указывающие на служебные строки
    SERVICE_KEYWORDS = [
...
class BatchClassEditor:
    ...
    def update_student_list(self, search_text="", class_filter="Все"):
        """Обновление списка учеников"""
        if not self.student_listbox:
            return

        self.student_listbox.delete(0, tk.END)
        self.student_items = []  # Для хранения соответствия индексов
        ...
        for student in self.parent.students:
            ...
            if class_filter != "Все" and student.student_class != class_filter:
                continue

            # Добавляем в список
            display_text = f"{student.name}"
            if student.student_class:
                display_text += f" [{student.student_class}]"

            self.student_listbox.insert(tk.END, display_text)
            self.student_items.append(student)
...
class SchoolAnalyticsApp:
    ...
    def debug_print_first_rows(self, num_rows=5):
        ...
        self.log(f"   Всего колонок: {len(self.data.columns)}")
        self.log(f"   Заголовки: {list(self.data.columns)}")

        for i in range(min(num_rows, len(self.data))):
            row = self.data.iloc[i]
            # Показываем первые 3 колонки для компактности
            preview = []
            for j, (col, val) in enumerate(zip(self.data.columns, row)):
                if j < 3:  # Показываем первые 3 колонки
                    preview.append(f"{col}: {val}")
                else:
                    preview.append("...")
                    break
            row_str = " | ".join(preview)
            self.log(f"  Строка {i + 1}: {row_str}")
    ...
    def load_file(self):
        ...
        try:
            ...
            # Очищаем текстовые колонки
            self.data = self.clean_text_columns(self.data)
            self.log(f"📊 После фильтрации: {len(self.data.columns)} колонок с оценками")
    ...
    def process_data(self):
        """Обработка загруженных данных с автоматическим объединением"""
        if self.data is None or self.data.empty:
        ...
        try:
            ...
            # Фильтруем сырые данные
            self.data = self.filter_raw_data(self.data)

            if len(self.data) == 0:
                self.log("❌ Нет данных после фильтрации")
                return
    ...
    def clean_text_columns(self, df):
        """Очистка текстовых колонок от лишних данных"""
        if df is None or df.empty:
            return df

        # Определяем, какие колонки содержат оценки
        grade_columns = []
        text_columns = []

        for col in df.columns[1:]:  # Пропускаем первую колонку (ФИО)
            ...
            # Считаем, сколько значений можно преобразовать в числа
            numeric_count = 0
            text_count = 0
            ...
            # Если больше 70% значений - числа, это колонка с оценками
        ...
        # Логируем найденные текстовые колонки
        if text_columns:
            self.log(f"📝 Найдены текстовые колонки (будут проигнорированы): {', '.join(text_columns[:5])}")
            if len(text_columns) > 5:
                self.log(f"   и еще {len(text_columns) - 5} колонок")

        # Оставляем только колонки с оценками
        columns_to_keep = [df.columns[0]] + grade_columns
    ...
    def paginate_class_analytics(self, class_name, students, page_size=20):
        """Разделение аналитики класса на страницы"""
        if len(students) <= page_size:
            return [students]

        pages = []
        for i in range(0, len(students), page_size):
            pages.append(students[i:i + page_size])

        return pages

    def show_class_analytics_paginated(self, class_name, page=1):
        """Отображение аналитики класса с пагинацией"""
        if class_name not in self.classes:
            return

        all_students = self.classes[class_name]
        pages = self.paginate_class_analytics(class_name, all_students)

        if page < 1 or page > len(pages):
            page = 1

        current_page_students = pages[page - 1]

        # Создаем временный класс для отображения
        temp_class = {class_name: current_page_students}
        ...
        pagination_text = f"\n\n📄 Страница {page} из {len(pages)} "
        pagination_text += f"(показано {len(current_page_students)} из {len(all_students)} учеников)"

        self.class_info_text.insert(tk.END, pagination_text)
    ...
    def process_classes_in_rows_temp(self, class_boundaries, original_data, name_col_idx=0):
        """Временная обработка файла с классами в отдельных строках"""
        students = []
        first_col = original_data.columns[name_col_idx]
        ...
        for boundary in class_boundaries:
            ...
            for idx in range(start_row, end_row + 1):
                ...
                if name and name.lower() not in ['nan', 'none', '']:
                    if not any(keyword in name.lower() for keyword in ['итого', 'средний', 'всего', 'класс']):
                        grades = DataFilter.extract_valid_grades(row, name_col_idx + 1)
                        student = Student(name, grades, class_name)
                        students.append(student)

        return students

    def process_classes_in_headers_temp(self, classes_data, original_data, name_col_idx=0):
        """Временная обработка файла с классами в заголовках"""
        students = []
    ...
    def process_standard_format_temp(self, class_col_idx, class_location, class_data, name_col_idx=0):
        """Временная обработка стандартного формата"""
        students = []
        first_col = self.data.columns[name_col_idx]
        class_info = class_data if class_location == 'header' else None
    ...
    def validate_data(self, df):
        ...
        if df is None:
            return False, "Данные отсутствуют (df is None)"

        if df.empty:
            return False, "DataFrame пуст"

        if len(df.columns) < 2:
            return False, f"В файле только {len(df.columns)} колонка. Нужно минимум 2 (ФИО + оценки)"

        # Проверяем первую колонку на наличие непустых значений
        first_col = df.columns[0]
        non_empty_mask = df[first_col].notna() & (df[first_col].astype(str).str.strip() != '')
        non_empty_count = non_empty_mask.sum()

        self.log(f"📊 Найдено непустых строк в первой колонке: {non_empty_count}")

        if non_empty_count == 0:
            return False, "Первая колонка не содержит имен учеников"

        # Показываем первые несколько имен для проверки
        sample_names = df[first_col][non_empty_mask].head(5).tolist()
        self.log(f"📋 Примеры имен: {sample_names}")

        # Проверяем, есть ли хоть одна колонка с числами
        numeric_columns = 0
        ...
        self.log(f"📊 Найдено колонок с числами: {numeric_columns}")

        if numeric_columns == 0:
            return False, "Не найдено колонок с числовыми значениями (оценками)"

        return True, f"Данные валидны. {non_empty_count} строк, {numeric_columns} колонок с числами"
