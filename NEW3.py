...
def filter_out_numeric_columns(df):
    """Фильтрует колонки, удаляя те, которые содержат номера учеников"""
    columns_to_drop = []
    ...
    for col in df.columns:
        ...
        if is_numeric_column(df[col]):
            # Дополнительно проверяем, что это не оценки
            sample_vals = df[col].dropna().head(10)
            if not any(val in [2, 3, 4, 5] for val in sample_vals):
                columns_to_drop.append(col)

    if columns_to_drop:
        st.info(f"Автоматически исключены колонки с номерами: {', '.join(map(str, columns_to_drop))}")
...
def parse_grades(df, grade_columns=None):
    ...
    VALID_GRADES = {'2', '3', '4', '5', 'н', 'Н', 'н/а', 'отсутствовал', '-', ''}
    ...
    if grade_columns is None:
        ...
        for col in grades_df.columns:
            if grades_df[col].dtype == 'object' or grades_df[col].dtype == 'float64':
                # Получаем уникальные значения в колонке (без NaN)
                unique_vals = grades_df[col].dropna().unique()

                # Проверяем, что все значения в колонке допустимы
                is_grade_column = True
                grade_count = 0

                for val in unique_vals:
                    val_str = str(val).strip().lower()
                    # Проверяем, является ли значение допустимой оценкой
                    if val_str in ['2', '3', '4', '5']:
                        grade_count += 1
                    elif val_str in ['н', 'н/а', 'отсутствовал', '-']:
                        grade_count += 1
                    elif _is_number(val_str) and float(val_str) in VALID_NUMERIC:
                        grade_count += 1
                    else:
                        # Если значение не является допустимой оценкой, проверяем его долю
                        # Если таких значений больше 20%, то это не колонка с оценками
                        non_grade_ratio = len(
                            [v for v in unique_vals if str(v).strip().lower() not in VALID_GRADES]) / len(unique_vals)
                        if non_grade_ratio > 0.2:  # Больше 20% недопустимых значений
                            is_grade_column = False
                            break

                if is_grade_column and grade_count > 0:
    ...
    # Преобразование в числовой формат с проверкой
    ...
    for col in grade_columns:
        ...
        for value in grades_df[col]:
            converted = convert_grade_to_number_with_check(value)
            if converted == 'invalid':
                invalid_in_col.append(value)
        ...
        if invalid_in_col:
            invalid_values[col] = list(set(invalid_in_col))  # Уникальные недопустимые значения

    # Если есть недопустимые значения, показываем предупреждение
    if invalid_values:
        st.warning("⚠️ Обнаружены недопустимые значения в колонках с оценками:")
        for col, values in invalid_values.items():
            st.write(f"• Колонка '{col}': {', '.join(str(v) for v in values[:5])}")
            if len(values) > 5:
                st.write(f"  и ещё {len(values) - 5} значений")
        st.info("Допустимы только оценки: 2, 3, 4, 5 и 'н' (отметка об отсутствии)")
...
def validate_grades_dataframe(grades_df):
    """Проверка всего DataFrame на допустимые оценки"""
    validation_results = {
...
def filter_valid_grades(grades_df):
    """Фильтрация DataFrame, оставляя только допустимые оценки"""
...
def get_detailed_statistics(grades_df, class_info=None):
    ...
    try:
        ...
        if not isinstance(grades_df, pd.DataFrame):
            return pd.DataFrame()
        ...
        for student in grades_df.index:
            try:
                ...
                if len(valid_numeric_grades) > 0:
                    ...
                    stats.append({
                        'ФИО': str(student),  # Сохраняем ФИО как отдельную колонку
                        ...
                        'Станд. отклонение': round(float(valid_numeric_grades.std()), 2) if len(
                            valid_numeric_grades) > 1 else 0,
                        ...
                        'Оценка 5': int((valid_numeric_grades == 5).sum()),
                        'Оценка 4': int((valid_numeric_grades == 4).sum()),
                        'Оценка 3': int((valid_numeric_grades == 3).sum()),
                        'Оценка 2': int((valid_numeric_grades == 2).sum()),
                    })
            except Exception as e:
        ...
        st.error(f"Критическая ошибка при получении статистики: {e}")
        return pd.DataFrame()
    ...
    if uploaded_file is not None:
        ...
        if df is not None:
            ...
            if name_column != 'Не выбрано':
                ...
                # Фильтруем колонки с номерами
                df_clean = filter_out_numeric_columns(df_clean)

                # Сохраняем чистые имена и устанавливаем их как индекс
                clean_names = df_clean['clean_name'].values
                df_clean = df_clean.drop(columns=['clean_name'])
                df_clean.index = clean_names

                # Показываем, какие ученики найдены
                st.success(f"✅ Найдено {len(df_clean)} учеников")
                with st.expander("Показать список учеников"):
                    st.write(list(df_clean.index))

                # Сбор информации о классах
                class_info = {}
                if class_column != 'Нет колонки с классами' and class_column in df.columns:
                    # Берем классы из исходного DataFrame, используя чистые имена
                    for idx, row in df.iterrows():
                        clean_name = clean_student_name(row[name_column])
                        if clean_name and clean_name in df_clean.index:
                            class_info[clean_name] = str(row[class_column]) if pd.notna(
                ...
                if grade_columns or st.button("Автоопределение оценок"):
                    ...
                    # В разделе "Анализ текущей успеваемости", после получения numeric_grades:
