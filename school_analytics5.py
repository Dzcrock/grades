...
class DataMergeDialog:
    ...
    def show(self):
        ...
        ttk.Button(button_frame, text="ОК",
                   command=lambda: self.on_ok(dialog)).pack(side=tk.RIGHT, padx=5)
...
class BatchClassEditor:
    ...
    def show_editor(self):
        ...
        class_filter_combo['values'] = ['Все'] + sorted([c for c in self.parent.classes.keys() if c])
...
class SchoolAnalyticsApp:
    ...
    def process_data(self):
        ...
        try:
            ...
            # Создаем реестр для объединения записей
            ...
            if class_location == 'separate_rows' and class_data:
                temp_students = self.process_classes_in_rows_temp(class_data, self.data, name_col_idx)
            elif class_location == 'header_rows' and class_data:
                temp_students = self.process_classes_in_headers_temp(class_data, self.data, name_col_idx)
            else:
                temp_students = self.process_standard_format_temp(class_col_idx, class_location, class_data,
                                                                  name_col_idx)

            # Объединяем записи через реестр
            self.students = []
            self.classes = {}

            for student in temp_students:
                registered = registry.add_student(student)
                if registered not in self.students:
                    self.students.append(registered)
            ...
            if self.students:
                ...
                total_originals = sum(len(s.original_names) for s in self.students)
                if total_originals > len(self.students):
                    self.log(f"🔄 Объединено {total_originals - len(self.students)} дублирующихся записей")
